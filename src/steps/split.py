...
def split_data(features_file, output_dir):
    os.makedirs(os.path.join(output_dir), exist_ok=True)

    data = pd.read_csv(features_file)

    with open('params.yaml', 'r') as params_file:
        params = yaml.safe_load(params_file)['split']
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 3:
        ...
        sys.stderr.write('\tpython3 split.py features_file output_dir \n')
        sys.exit(1)

    features_file = sys.argv[1]
    output_dir = sys.argv[2]

    split_data(features_file, output_dir)
